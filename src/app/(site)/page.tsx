...
// Public hotel content changes infrequently, so let Vercel serve the cached
// homepage for five minutes before regenerating it in the background.
export const revalidate = 300;

const featuredCategories = ['hotel', 'rooms', 'restaurant', 'pool', 'exterior'] as const;
const featuredCounts = { hotel: 1, rooms: 2, restaurant: 2, pool: 1, exterior: 2 } as const;
...
async function getHomeData() {
  ...
  const [settingsResult, roomsResult, testimonialsResult, galleryResult] = await Promise.all([
    ...
    supabase.from('rooms').select('*, room_images(*)').eq('active', true).eq('archived', false).order('featured', { ascending: false }).order('price_per_night', { ascending: true }),
    supabase.from('testimonials').select('*').eq('published', true).order('featured', { ascending: false }).limit(3),
    supabase.from('gallery').select('*').in('category', [...featuredCategories, 'setting_1', 'setting_2']).order('display_order', { ascending: true }).order('created_at', { ascending: true }),
  ]);

  const settings = (settingsResult.data as HotelSettings | null) ?? null;
  const allRooms = (roomsResult.data as RoomWithImages[]) ?? [];
  const rooms = allRooms.filter((room) => room.featured).slice(0, 3);
  ...
  const featuredGallery = featuredCategories.flatMap((category) => {
    const items = gallery.filter((item) => item.category === category).slice(0, featuredCounts[category]);
    categoryIndexes[category] = items.length;
    return items;
  });

  const settingImages = {
    one: gallery.find((item) => (item.category as string) === 'setting_1')?.storage_path ?? null,
    two: gallery.find((item) => (item.category as string) === 'setting_2')?.storage_path ?? null,
  };

  return { settings, rooms, roomTypes, testimonials, settingImages, featuredGallery };
}
...
const featuredLabels: Record<GalleryItem['category'], string> = {
  hotel: 'Hotel', rooms: 'Rooms', restaurant: 'Restaurant', pool: 'Pool', exterior: 'Exterior', facilities: 'Facilities', events: 'Events',
};

export default async function HomePage() {
  const { settings, rooms, roomTypes, testimonials, settingImages, featuredGallery } = await getHomeData();
    ...
    <>
      ...
      {featuredGallery.length > 0 && (
        <section className="bg-white py-24"><div className="container-lake">
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{featuredGallery.map((item) => { const index = categoryIndexes[item.category] ?? 0; const copies = featuredGalleryCopy[item.category]; const copy = copies[index] ?? copies[0]; categoryIndexes[item.category] = index + 1; const title = item.caption ?? copy.title; return <article key={item.id} className="group overflow-hidden rounded-xl border border-sand bg-mist"><div className="relative aspect-[4/3] overflow-hidden bg-still"><Image src={resolveImageUrl(item.storage_path, 'gallery-images')} alt={title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" quality={75} /><span className="absolute left-3 top-3 rounded-full bg-reservoir/90 px-3 py-1 text-[11px] font-medium uppercase tracking-wider text-white">{featuredLabels[item.category]}</span></div><div className="p-5"><h3 className="font-display text-xl text-ink">{title}</h3><p className="mt-2 text-sm leading-relaxed text-ink/60">{copy.description}</p></div></article>; })}</div>
