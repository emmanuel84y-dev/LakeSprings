...
import { Star, Trash2, ChevronUp, ChevronDown, Upload, Loader2, AlertCircle } from 'lucide-react';
import { resolveImageUrl, cn } from '@/lib/utils';
import { uploadRoomImage, deleteRoomImage, setPrimaryRoomImage, reorderRoomImage } from '@/lib/actions/rooms-admin';
...
export function RoomImageManager({ roomId, images }: { roomId: string; images: RoomImage[] }) {
  ...
  return (
    <div>
      ...
      <p className="mt-1.5 text-xs text-ink/40">JPEG, PNG, or WebP — up to 5MB.</p>
      ...
      {sorted.length > 0 ? (
        ...
        <p className="mt-5 text-sm text-ink/50">No images yet — the first upload becomes the primary image automatically.</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg border border-sand">
      <div className="relative aspect-[4/3]">
        ...
        {image.is_primary && (
          <span className="absolute left-2 top-2 rounded-full bg-brass px-2 py-0.5 text-[10px] font-medium text-white">Primary</span>
        )}
      </div>
      <div className="flex items-center justify-between gap-1 bg-white px-2 py-1.5">
        <div className="flex gap-0.5">
          <IconButton
            ...
            onClick={() => startTransition(() => reorderRoomImage(roomId, image.id, 'up'))}
          >
          ...
          <IconButton
            ...
            onClick={() => startTransition(() => reorderRoomImage(roomId, image.id, 'down'))}
          >
        ...
        <div className="flex gap-0.5">
          <IconButton
            ...
            onClick={() => startTransition(() => setPrimaryRoomImage(roomId, image.id))}
          >
            <Star className={cn('h-3.5 w-3.5', image.is_primary && 'fill-brass text-brass')} />
          </IconButton>
          <IconButton
            title="Delete"
            disabled={pending}
            onClick={() => {
              if (confirm('Remove this image?')) startTransition(() => deleteRoomImage(roomId, image.id, image.storage_path));
            }}
          >
