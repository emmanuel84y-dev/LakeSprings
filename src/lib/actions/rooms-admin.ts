...
export async function createRoom(formData: FormData) {
  ...
  // RLS silently returns "no rows" rather than a permissions error for
  // policy failures on INSERT ... RETURNING, so we treat a missing row
  // the same as a thrown error and never claim success either way.
  if (error || !created) {
    return { ok: false, error: error?.code === '23505' ? 'That slug is already in use' : 'Could not create the room' };
  }

  if (amenity_ids.length) {
    await supabase.from('room_amenities').insert(amenity_ids.map((amenity_id) => ({ room_id: created.id, amenity_id })));
  }
...
export async function updateRoom(roomId: string, formData: FormData) {
  ...
  if (error) {
    return { ok: false, error: error.code === '23505' ? 'That slug is already in use' : 'Could not save changes' };
  }

  await supabase.from('room_amenities').delete().eq('room_id', roomId);
  if (amenity_ids.length) {
    await supabase.from('room_amenities').insert(amenity_ids.map((amenity_id) => ({ room_id: roomId, amenity_id })));
  }

  // A price change here is the whole point of §25 of the spec: the
  // moment this commits, /rooms/[slug] reads the new value straight
  // from Supabase — there is no cached/hardcoded price anywhere.
  revalidatePath('/admin/rooms');
  revalidatePath('/rooms');
  revalidatePath('/');
  return { ok: true };
...
export async function archiveRoom(roomId: string) {
  const supabase = createClient();
  // Archive, don't delete — preserves historical bookings per §23.
